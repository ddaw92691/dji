...
export default function OrderDetailPage() {
  ...
  const handleConfirm = async () => {
    if (!order || !window.confirm('Confirm receipt?')) return
    try {
    ...
    return (
      <div className="flex flex-col min-h-screen">
        ...
        <div className="flex flex-col items-center justify-center flex-1 gap-3 text-gray-400">
          ...
          <button onClick={() => navigate('/orders')} className="text-sm text-blue-500">View Orders</button>
        </div>
      </div>
    )
  }
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="p-4 bg-white border-b">
          ...
          {order.paidAt && (
            <p className="text-xs text-gray-400">Paid: {new Date(order.paidAt).toLocaleString()}</p>
          )}
          {order.cancelledAt && (
            <p className="text-xs text-red-400">Cancelled: {new Date(order.cancelledAt).toLocaleString()}</p>
          )}
        </div>
                ...
                return (
                  <div key={step} className="flex items-center flex-1">
                    ...
                    <div className="flex flex-col items-center">
                      ...
                      <span className="text-[10px] text-gray-400 mt-0.5">{STATUS_LABELS[step]}</span>
                    </div>
                  </div>
                )
              })}
        ...
        {order.remark && (
          <div className="p-4 bg-white border-b">
            <h3 className="text-xs text-gray-500 mb-1">Remark</h3>
        ...
        {(order.status === 'SHIPPED' || order.status === 'COMPLETED') && (order.logisticsCompany || order.trackingNo) && (
          <div className="p-4 bg-white border-b">
            <h3 className="text-xs text-gray-500 mb-2">Logistics</h3>
            <div className="space-y-1 text-sm">
              {order.logisticsCompany && (
                <p className="text-gray-700">Carrier: {order.logisticsCompany}</p>
              )}
              {order.trackingNo && (
                <p className="text-gray-700">Tracking: <span className="font-mono">{order.trackingNo}</span></p>
              )}
            </div>
          </div>
        )}
        ...
        {(order.refundStatus === 'REQUESTED' || order.refundStatus === 'APPROVED' || order.refundStatus === 'REJECTED') && (
          <div className="p-4 bg-white border-b">
            <h3 className="text-xs text-gray-500 mb-2">Refund</h3>
            <div className="space-y-1 text-sm">
              {order.refundAmount > 0 && (
                <p className="text-gray-700">Amount: <span className="font-medium">¥{order.refundAmount.toFixed(2)}</span></p>
              )}
              {order.refundReason && (
                <p className="text-gray-700">Reason: {order.refundReason}</p>
              )}
              <p className="text-gray-700">Status: <span className={`font-medium ${
                ...
                'text-yellow-600'
              }`}>{order.refundStatus}</span></p>
              {order.refundRejectReason && (
                <p className="text-red-500">Reject reason: {order.refundRejectReason}</p>
              )}
            </div>
          </div>
        )}
        ...
        <div className="p-4 bg-white border-b">
          <h3 className="text-xs text-gray-500 mb-2">Items</h3>
                    ...
                    <div className="mt-1">
                      {(item as any).reviewed ? (
                        <span className="inline-flex items-center gap-1 text-xs text-green-600 bg-green-50 px-1.5 py-0.5 rounded">
                          ...
                          Reviewed {(item as any).reviewRating > 0 ? (item as any).reviewRating : ''}
                        </span>
                      ) : (
                        <button
                          onClick={(e) => {
