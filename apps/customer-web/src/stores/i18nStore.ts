...
async function fetchMessages(l: LocaleEntry): Promise<Dict> {
  try {
    const res = await publicApi.getI18n(l.countryCode, l.languageCode, NAMESPACES)
...
export const useI18nStore = create<I18nState>((set, get) => ({
  ...
  setLocale: async (countryCode: string, languageCode: string) => {
    const match =
      LOCALES.find((l) => l.countryCode === countryCode && l.languageCode === languageCode) ||
      get().locales.find((l) => l.languageCode === languageCode)
    const locale: LocaleEntry = match || { ...initial, countryCode, languageCode }
    persistLocale(locale)
    set({ localeId: locale.id, countryCode: locale.countryCode, languageCode: locale.languageCode, loading: true })
