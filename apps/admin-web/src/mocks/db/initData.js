 ...
 * 初始化 IndexedDB 默认数据
 */
import { addUser, userExists, STORES, add, getAll, update, ensureDBInitialized, } from './index';
 ...
 */
function getMenuDescendants(menuId, allMenus) {
    const descendants = [];
...
export async function initDefaultRoles() {
    try {
        // 检查是否已有角色数据
        const existingRoles = await getAll(STORES.ROLES);
        if (existingRoles.length === 0) {
            ...
            const defaultRoles = APP_CONFIG.mock.defaultRoles.map((role) => ({
                ...role,
                menuIds: role.code === 'super_admin'
                    ? allMenuIds
                    : role.code === 'user'
...
function collectMenuPaths(menuItems, acc = new Set()) {
 ...
 * 将树形菜单数据转换为扁平结构
 */
function flattenMenuTree(menuItems, parentId = null, order = 0) {
    const menus = [];
 ...
 * 初始化默认菜单数据
 */
export async function initDefaultMenus() {
    try {
        // 检查是否已有菜单数据
        const existingMenus = await getAll(STORES.MENUS);
        ...
        if (existingMenus.length > 0) {
            ...
            for (const menu of existingMenus) {
                ...
                if (!menu.type) {
                    ...
                    const defaultType = hasChild ? 'directory' : 'menu';
