...
import { getAll, add, update, remove, getById, STORES, getUserById, usernameExists, buildMenuTree, getMenuAncestors, } from '../db/index';
...
export const getUserListHandler = http.get(`${MSW_BASE}/users`, async ({ request }) => {
    ...
    try {
        ...
        // 从 IndexedDB 获取所有用户
        let users = await getAll(STORES.USERS);
        // 筛选
        if (username) {
...
export const updateUserHandler = http.put(`${MSW_BASE}/users`, async ({ request }) => {
    ...
    try {
        ...
        await update(STORES.USERS, updatedUser);
        return HttpResponse.json({
            code: 200,
            message: '更新成功',
            data: updatedUser,
 ...
 * 从token中获取用户ID，无需路径参数
 */
export const getUserPermissionsHandler = http.get(`${MSW_BASE}/users/permissions`, async ({ request }) => {
    // 验证token并获取用户ID
    const { error, userId } = verifyAuth(request);
    if (error) {
        return error;
    ...
    try {
        ...
        // 获取角色信息
        const role = await getById(STORES.ROLES, user.roleId);
        if (!role) {
            return HttpResponse.json({
                code: 200,
                message: '获取成功',
        ...
        // 获取所有菜单
        const allMenus = await getAll(STORES.MENUS);
        // 根据menuIds过滤，只获取用户有权限的菜单，且status为active
        const userMenuIds = new Set(role.menuIds);
        // 对于每个有权限的菜单（包括按钮、menu、directory），自动包含其所有父菜单
        const userAuthorizedMenus = allMenus.filter((menu) => role.menuIds.includes(menu.id) && menu.status === 'active');
        ...
        const userMenus = allMenus.filter((menu) => userMenuIds.has(menu.id) && menu.status === 'active');
        // 分离菜单和按钮权限
        const menuItems = userMenus.filter((menu) => menu.type === 'directory' || menu.type === 'menu');
        const userButtonMenus = userMenus.filter((menu) => menu.type === 'button');
        // 构建菜单树
        const menuTree = buildMenuTree(menuItems);
        // 提取按钮权限并去重
        const buttonPermissions = Array.from(new Set(userButtonMenus
 ...
 * 从token中获取用户ID，无需路径参数
 */
export const updateUserProfileHandler = http.put(`${MSW_BASE}/users/profile`, async ({ request }) => {
    // 验证token并获取用户ID
    const { error, userId } = verifyAuth(request);
    if (error) {
        return error;
 ...
 * 从token中获取用户ID，无需路径参数
 */
export const updateUserPasswordHandler = http.put(`${MSW_BASE}/users/password`, async ({ request }) => {
    // 验证token并获取用户ID
    const { error, userId } = verifyAuth(request);
    if (error) {
        return error;
