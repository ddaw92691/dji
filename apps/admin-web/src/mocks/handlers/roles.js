...
import { getAll, getById, add, update, remove, STORES, roleCodeExists, } from '../db/index';
...
export const getRoleListHandler = http.get(`${MSW_BASE}/roles`, async ({ request }) => {
    ...
    try {
        ...
        // 从 IndexedDB 获取所有角色
        let roles = await getAll(STORES.ROLES);
        // 筛选
        if (name) {
...
export const getRoleByIdHandler = http.get(`${MSW_BASE}/roles/:id`, async ({ params, request }) => {
    ...
    try {
        ...
        const role = await getById(STORES.ROLES, id);
        if (!role) {
            return HttpResponse.json({
                code: 500,
                message: '角色不存在',
...
export const createRoleHandler = http.post(`${MSW_BASE}/roles`, async ({ request }) => {
    ...
    try {
        ...
        await add(STORES.ROLES, newRole);
        return HttpResponse.json({
            code: 200,
            message: '创建成功',
            data: newRole,
...
export const updateRoleHandler = http.put(`${MSW_BASE}/roles`, async ({ request }) => {
    ...
    try {
        ...
        // 获取现有角色
        const existingRole = await getById(STORES.ROLES, body.id);
        if (!existingRole) {
            return HttpResponse.json({
                code: 500,
                message: '角色不存在',
        ...
        const updatedRole = {
            ...
            menuIds: body.menuIds !== undefined
        ...
        await update(STORES.ROLES, updatedRole);
        return HttpResponse.json({
            code: 200,
            message: '更新成功',
            data: updatedRole,
