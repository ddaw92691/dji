...
import { getById, add, update, remove, getAll, STORES, menuPathExists, hasChildren, buildMenuTree, getMenuAncestors, } from '../db/index';
 ...
 * 支持后端搜索过滤
 */
export const getMenuListHandler = http.get(`${APP_CONFIG.listenMSWPath}/menus`, async ({ request }) => {
    // 验证token
    ...
    try {
        ...
        if (title || path || type || status) {
            ...
            // 收集所有匹配菜单及其祖先菜单的ID
            const menuIdsToInclude = new Set();
 ...
 * 获取菜单详情
 */
export const getMenuByIdHandler = http.get(`${APP_CONFIG.listenMSWPath}/menus/:id`, async ({ params, request }) => {
    // 验证token
    const { error } = verifyAuth(request);
    if (error) {
        return error;
    ...
    try {
        ...
        const menu = await getById(STORES.MENUS, id);
        if (!menu) {
            return HttpResponse.json({
                code: 500,
                message: '菜单不存在',
 ...
 * 创建菜单
 */
export const createMenuHandler = http.post(`${APP_CONFIG.listenMSWPath}/menus`, async ({ request }) => {
    // 验证token
    const { error } = verifyAuth(request);
    if (error) {
        return error;
    ...
    try {
        ...
        await add(STORES.MENUS, newMenu);
        return HttpResponse.json({
            code: 200,
            message: '创建成功',
            data: null,
 ...
 * 更新菜单
 */
export const updateMenuHandler = http.put(`${APP_CONFIG.listenMSWPath}/menus`, async ({ request }) => {
    // 验证token
    const { error } = verifyAuth(request);
    if (error) {
        return error;
    ...
    try {
        ...
        // 获取现有菜单
        const existingMenu = await getById(STORES.MENUS, id);
        if (!existingMenu) {
            return HttpResponse.json({
                code: 500,
                message: '菜单不存在',
        ...
        await update(STORES.MENUS, updatedMenu);
        return HttpResponse.json({
            code: 200,
            message: '更新成功',
            data: null,
 ...
 * 删除菜单
 */
export const deleteMenuHandler = http.delete(`${APP_CONFIG.listenMSWPath}/menus/:id`, async ({ params, request }) => {
    // 验证token
    const { error } = verifyAuth(request);
    if (error) {
        return error;
