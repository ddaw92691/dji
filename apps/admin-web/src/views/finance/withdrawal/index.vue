<template>
  <div class="withdrawal-page">
    <el-form :inline="true" :model="searchForm" class="search-bar">
      ...
      <el-form-item>
        <el-select v-model="searchForm.role" placeholder="Role" clearable @change="handleSearch">
    ...
    <el-table :data="tableData" border stripe v-loading="loading">
      ...
      <el-table-column prop="bankName" label="Bank" width="120" show-overflow-tooltip />
      <el-table-column prop="bankAccount" label="Account" width="160" show-overflow-tooltip />
      <el-table-column prop="createdAt" label="Created" width="180" />
      <el-table-column label="Actions" width="240" fixed="right">
        <template #default="{ row }">
          <el-button link type="primary" v-permission="'finance:withdrawal:view'" @click="openDetail(row.id)">Detail</el-button>
          <el-button v-if="row.status === 'PENDING'" link type="success" v-permission="'finance:withdrawal:approve'" @click="handleApprove(row)">Approve</el-button>
          <el-button v-if="row.status === 'PENDING'" link type="danger" v-permission="'finance:withdrawal:reject'" @click="openRejectDialog(row)">Reject</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog v-model="detailVisible" title="Withdrawal Detail" width="600px">
      <el-descriptions v-if="detail" :column="2" border size="small">
        <el-descriptions-item label="ID">{{ detail.id }}</el-descriptions-item>
        <el-descriptions-item label="User">{{ detail.userName }}</el-descriptions-item>
        <el-descriptions-item label="Role">{{ detail.role }}</el-descriptions-item>
        <el-descriptions-item label="Type">{{ detail.type }}</el-descriptions-item>
        <el-descriptions-item label="Amount">${{ (detail.amount ?? 0).toFixed(2) }}</el-descriptions-item>
        ...
        <el-descriptions-item label="Bank Name">{{ detail.bankName || '-' }}</el-descriptions-item>
        <el-descriptions-item label="Bank Account">{{ detail.bankAccount || '-' }}</el-descriptions-item>
        <el-descriptions-item label="Account Name">{{ detail.accountName || '-' }}</el-descriptions-item>
        <el-descriptions-item label="Remark">{{ detail.remark || '-' }}</el-descriptions-item>
        <el-descriptions-item label="Reject Reason" :span="2">{{ detail.rejectReason || '-' }}</el-descriptions-item>
        <el-descriptions-item label="Created">{{ detail.createdAt }}</el-descriptions-item>
    ...
    <el-dialog v-model="rejectVisible" title="Reject Withdrawal" width="480px">
      <el-form :model="rejectForm">
        <el-form-item label="Reject Reason" required>
          <el-input v-model="rejectForm.rejectReason" type="textarea" :rows="4" placeholder="Please enter reject reason" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="rejectVisible = false">Cancel</el-button>
        <el-button type="danger" :loading="rejectLoading" @click="handleReject">Confirm Reject</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
async function handleApprove(row: IAdminWithdrawal) {
  ...
  if (res.code !== 200) return
  ElMessage.success('Withdrawal approved')
  fetchData()
}
...
async function handleReject() {
  ...
  try {
    ...
    if (res.code !== 200) return
    ElMessage.success('Withdrawal rejected')
