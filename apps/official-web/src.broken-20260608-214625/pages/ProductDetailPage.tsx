...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    if (!id) return
    api.getProductDetail(Number(id)).then(r => { if (r.data.code === 200) setProduct(r.data.data) }).finally(() => setLoading(false))
  }, [id])
  ...
  if (!product) return <div className="min-h-screen bg-black flex items-center justify-center text-gray-400">Product not found</div>
          ...
          <div className="grid md:grid-cols-2 gap-8 md:gap-16">
            <div>
              <div className="aspect-square rounded-2xl overflow-hidden bg-white/5">
                <img src={product.images?.[mainImg]?.imageUrl || product.coverImage}
            ...
            <div className="flex flex-col justify-center">
              ...
              <div className="mt-4 text-sm text-gray-500">
                ...
                <span className="ml-4">Category: {product.categoryName}</span>
              </div>
              <div className="mt-8 flex gap-4">
                <a href={`http://localhost:4173/products/${id}`} target="_blank" rel="noopener"
