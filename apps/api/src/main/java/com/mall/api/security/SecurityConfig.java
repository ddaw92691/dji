    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        // 放行所有 CORS 预检请求（OPTIONS 不携带凭证，必须在鉴权之前放行）
                        .requestMatchers(HttpMethod.OPTIONS, "/**").permitAll()
                        .requestMatchers("/api/health").permitAll()
                        .requestMatchers("/api/auth/**").permitAll()
                        .requestMatchers("/api/public/**").permitAll()
                        .requestMatchers("/api/v1/public/**").permitAll()
                        .requestMatchers(HttpMethod.POST, "/api/v1/merchant/dealer-applications").permitAll()
                        .requestMatchers(HttpMethod.GET, "/api/v1/merchant/dealer-applications/*/status").permitAll()
                        .requestMatchers(HttpMethod.POST, "/api/v1/merchant/upload").permitAll()
                        ...
                        .requestMatchers("/api/merchant/withdraw-accounts/**", "/api/merchant/recharges/**").hasRole("MERCHANT")
                        .requestMatchers("/api/v1/merchant/orders/**", "/api/v1/merchant/withdraw-accounts/**", "/api/v1/merchant/recharges/**").hasRole("MERCHANT")
                        ...
                        .requestMatchers("/api/admin/recharges/**").hasAnyRole("SUPER_ADMIN", "ADMIN")
                        ...
                        // 敏感证件/手持视频禁止静态直链访问，只能经 /api/admin/merchant-applications/{id}/file/{field} 鉴权下载
                        .requestMatchers("/uploads/merchant_application/**").denyAll()
